	...
	<body>
		...
		<div id="show-topic" class="content scaffold-show" role="main">
			...
			<ol class="property-list topic">
				...
				<g:if test="${topicInstance?.subscritions}">
				<li class="fieldcontain">
					<span id="subscritions-label" class="property-label"><g:message code="topic.subscritions.label" default="Subscritions" /></span>
					
						<g:each in="${topicInstance.subscritions}" var="s">
						<span class="property-value" aria-labelledby="subscritions-label"><g:link controller="subscription" action="show" id="${s.id}">${s?.encodeAsHTML()}</g:link></span>
						</g:each>
